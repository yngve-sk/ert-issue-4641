...
class FlowRun:
    ...
    The FlowRun class is a small wrapper class which is used to run FLOW
    simulations. It will load a configuration, i.e. where the binary is
    installed and so on, from an instance of the FlowRun class.

    The main method is the runFlow() method which will:
    ...
    If the simulation fails the runFlow() method will raise an exception.

    The class is called FlowRun, and the main focus has been on running Eclipse
    ...
    def _get_run_command(self, flowrun_config: EclrunConfig):
        summary_conversion = "yes" if self.summary_conversion else "no"
        return [
            "eclrun",
            "-v",
            flowrun_config.version,
            flowrun_config.simulator_name,
    ...
    def _get_log_name(self, flowrun_config=None):
        # Eclipse version >= 2019.3 should log to a .OUT file
        # and not the legacy .LOG file.
        eclipse_version = (
            self.sim.version if flowrun_config is None else flowrun_config.version
        )
    ...
    def execEclipse(self, flowrun_config=None) -> int:
        use_eclrun = flowrun_config is not None
        log_name = self._get_log_name(flowrun_config=flowrun_config)
        ...
        with pushd(self.run_path), open(log_name, "wb") as log_file:
            ...
            command = (
                self._get_run_command(flowrun_config)
                if use_eclrun
                else self._get_legacy_run_command()
            )
            env = flowrun_config.run_env if use_eclrun else self._get_legacy_run_env()
    ...
    def runFlow(self, flowrun_config=None, retries_left=3, backoff_sleep=None):
        backoff_sleep = (
            self.LICENSE_FAILURE_RETRY_INITIAL_SLEEP
            if backoff_sleep is None
            else backoff_sleep
        )
        return_code = self.execEclipse(flowrun_config=flowrun_config)
