...
@dataclass
class _GenObsDataset:
    gen_data_keys: List[str] = field(default_factory=list)
    observations: List[float] = field(default_factory=list)
    stds: List[float] = field(default_factory=list)
    indexes: List[int] = field(default_factory=list)
    report_steps: List[int] = field(default_factory=list)
    obs_names: List[str] = field(default_factory=list)
        ...
        for vec in vecs:
            if vec.observation_type == ResponseTypes.GEN_DATA:
                for report_step, node in vec.observations.items():
                    # wrt typing: Hard to prove to mypy
                    # that this generic typing works
                    # ideally this will be abstracted
                    # a bit differently so typing resolves more nicely
                    gen_obs.write(
