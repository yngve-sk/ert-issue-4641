...
class PolarsHooksParquet(DatalibHooks):
    @staticmethod
    @profile_me
    ...
    def query_single_summary_key_for_all_realizations(
        ...
        if os.path.exists(ds_file):
            ds = pl.scan_parquet(ds_file)
            pddf = (
                ds.filter((pl.col("response_key") == response_key))
                .select(["realization", "time", "value"])
                .collect()
            )
    ...
    def join_observations_and_responses(
        ...
        responses_ds = pl.read_parquet(responses_ds_file)

        # Can't pivot lazy frame
        obs_ds = pl.read_parquet(
            observations_file.replace(".polars_parquet", ".parquet")
        )
    ...
    def combine_one_ds_per_response_key(
        ...
        for response_key in response_keys:
            for i in range(num_reals):
                fp = f"{p_real(i)}/{response_key}.polars_parquet"
                if os.path.exists(fp):
                    ds = pl.scan_parquet(
                        f"{p_real(i)}/{response_key}.polars_parquet"
                    ).with_columns(
                        [
        ...
        concatd.collect().write_parquet(p_out / "gen_data_combined.polars_parquet")
    ...
    def combine_one_ds_many_response_keys(
        ...
        for i in range(num_reals):
            ...
            if os.path.exists(fp):
                ds = pl.scan_parquet(fp).with_columns([pl.lit(i).alias("realization")])
                all_ds.append(ds)

        concatd = pl.concat(all_ds)
        concatd.collect().write_parquet(p_out / "summary_combined.polars_parquet")
    ...
    def query_single_realization_for_all_existing_responses(
        ...
        unique_response_names_list = (
            filtered_df.select("response_key").unique().collect().to_series().to_list()
        )
    ...
    def query_all_realizations_with_all_responses(
        ...
        df = pl.scan_parquet(ds_file).select(["realization", "response_key"])
        ...
        result = (
            response_key_count.filter(
                pl.col("response_key_count")
                == total_keys.collect()["total_response_keys"][0]
            )
            .select("realization")
            .collect()
        )
    ...
    def from_dataframe_to_file(
        ...
        polars_df.write_parquet(file=output_filename)
        t1 = time.time()
